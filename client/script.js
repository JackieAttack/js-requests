...
// CODE HERE
function helloColor() {
    sayHelloButton.classList.add('blackWhite')
}
...
// CODE HERE
function helloColorBack() {
    sayHelloButton.classList.remove('blackWhite')
}

sayHelloButton.addEventListener("mouseout", helloColorBack)
...
// CODE HERE
function makeQuery() {
    axios.get('http://localhost:3000/query-test?name=Jackie&lastname=Bearnson')
        .then((res) => {
            console.log(res.data)
        })
        .catch((err) => {
...
queryBtn.addEventListener("click", makeQuery)
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        ...
        It console logged "You sent an empty query!" (http://localhost:3000/query-test?)

    2: Send more than 1 query on the URL -- what happened? 

    It said I sent more than one query. (http://localhost:3000/query-test?name=Jackie&lastname=Bearnson)
*/
...
function createFood() {
    ...
    axios.post('http://localhost:3000/food', body)
        .then((res) => {
            console.log(res.data);
            document.querySelector('#food-display').textContent = res.data;
        })
        .catch((err) => {
            console.log(err)
        })
...
foodBtn.addEventListener("submit", createFood)